...
struct VPageIndicatorInfinite: View {
    ...
    public var body: some View {
        ...
        case (true, ...visible): VPageIndicatorFinite(
                model: model,
                total: total,
    ...
    private var offset: CGFloat {
        ...
        case .center:
            let incrementalOffset: CGFloat = -.init(selectedIndex - middle) *
                (model.layout.dotDimension + model.layout.spacing)
    ...
    private func scale(at index: Int) -> CGFloat {
        switch region {
        case .leftEdge:
            guard let leftEdgeVisibleIndex: Int = leftEdgeVisibleIndex(at: index),
        ...
        case .center:
            guard let visibleIndex: Int = centerVisibleIndex(at: index),
        ...
        case .rightEdge:
            guard let rightEdgeVisibleIndex: Int = rightEdgeVisibleIndex(at: index),
                  let rightEdgeleftSideIndex: Int =
                  rightEdgeleftSideIndex(at: rightEdgeVisibleIndex)
            else {
                return 1
            }
...
// MARK: - VPageIndicatorInfinite_Previews
