...
// MARK: - BMChineseSort + BMChineseSortProtocol
...
extension BMChineseSort: BMChineseSortProtocol {
    ...
    public static func sortAndGroup<T>(
        objectArray: [T]?,
        key: String?,
        finish: @escaping (
            _ success: Bool,
    ...
    func sortAndGroup<T>(
        ...
        // 返回空
        if objectArray == nil || objectArray!.isEmpty {
            finish(true, [], [], [])
            return
        }
        // key是否正确
        ...
        if firstoObj is String {
            if key != nil, key?.isEmpty != true {
                logMsg("** warning ** :当前排序对象为字符串类型，key无法生效")
            }
        } else {
            // 对象类型没传Key
            if key == nil {
                logMsg("** error ** :数组内元素不是字符串类型,如果是对象类型，请传key")
                finish(false, [], [], [])
                return
            }
            // key 是否存在
            ...
            if containKey == false {
                logMsg("** error ** :请确认当前模型是否包含对应key值")
                finish(false, [], [], [])
                return
            }
        }
        ...
        DispatchQueue.global().async {
            ...
            var sortModelArray = [BMChineseSortModel<T>]()
...
//            self.logMsg(String(format: "排序用时：%f s", state3-state2))
            ...
            for model in sortModelArray {
                var firstLetter = String(model.pinYin.prefix(1))
...
//            self.logMsg(String(format: "分组用时：%f s", state3-state2))
    ...
    private func getModelWithObj<T>(_ obj: T, key: String?) -> BMChineseSortModel<T>? {
        ...
        if model.string.isEmpty {
            model.string = specialCharSectionTitle
        } else {
            let prefix = String(model.string.prefix(1))
            if ignoreModelWithPrefix.contains(prefix) {
                return nil
            } else {
                // 拿到拼音
                ...
                if sortMode == 1 {
                    model.pinYin = getFirstLetter(model.string)
                } else {
                    model.pinYin = getFirstLetter2(model.string)
                }
            }
        }
        return model
    ...
    private func getFirstLetter(_ chinese: String) -> String {
        ...
        for (key, value) in polyphoneMapping {
        ...
        if c >= "A", c <= "Z" { //
            ...
            return specialCharSectionTitle
        }
    }
