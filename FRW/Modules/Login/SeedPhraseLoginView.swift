...
struct SeedPhraseLoginView: RouteableView {
    ...
    @StateObject var viewModel = SeedPhraseLoginViewModel()
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 16) {
                    ...
                    Section() {
                        ...
                        ImportTextView(content: $viewModel.words, placeholder: "seedphrase_hint".localized) { value in
                    ...
                    VStack(spacing: 16) {
                        Section() {
                            AnimatedSecureTextField(placeholder: "Derivation Path".localized + " (m/44'/539'/0'/0/0)", text: $viewModel.derivationPath) { text in
                                viewModel.updateState()
                            }
                                .frame(height: 64)
                        ...
                        Section() {
                            AnimatedSecureTextField(placeholder: "Passphrase".localized + " " + "Optional".localized, text: $viewModel.passphrase) { text in
                                viewModel.updateState()
                            }
                                .frame(height: 64)
