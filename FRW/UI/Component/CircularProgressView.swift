...
struct CircularProgressIndicator: View {
    ...
    var body: some View {
        Circle()
            ...
            .rotation(.degrees(-90))
            .stroke(Color.Theme.Accent.green, style: .init(lineWidth: animationPhase.values.lineWidth, lineCap: .round))
            .shadow(color: .black.opacity(animationPhase == .sealed ? 0 : 0.1), radius: 10, x: 0, y: 5)
            .animation(.bouncy, value: animationPhase)
    }
}
